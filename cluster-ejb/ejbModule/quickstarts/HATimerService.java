...
public class HATimerService implements Service<String> {
    ...
    public void start(StartContext arg0) throws StartException {
        ...
        try {
            ...
            ((Scheduler) ic.lookup("global/ClusterEar/cluster-ejb/SchedulerBean!quickstarts.Scheduler")).initialize("HASingleton timer @" + this.nodeName + " " + new Date());
    ...
    public void stop(StopContext arg0) {
        if (!started.compareAndSet(true, false)) {
            LOGGER.warn("T--------he service '" + this.getClass().getName() + "' is not active!");
        } else {
            LOGGER.info("------------Stop HASingleton timer service '" + this.getClass().getName() + "'");
            try {
                InitialContext ic = new InitialContext();
                ((Scheduler) ic.lookup("global/ClusterEar/cluster-ejb/SchedulerBean!quickstarts.Scheduler")).stop();
            } catch (NamingException e) {
